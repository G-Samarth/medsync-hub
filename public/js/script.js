...
window.addEventListener("scroll", function () {
  ...
  if (window.scrollY < 100) $(".btn-dange").hide();
  else {
    $(".btn-dange").show();
  }
  $(".toShow").show();
...
function change() {
  ...
  if (!lightFlag) {
    $("body").css("background-image", "url('./img/bgph.jpeg')");
    // $(".iphoneDark").attr("src", "IMAGE272.png");
...
function myFunction() {
  ...
  fetch(
    "https://heartapi.herokuapp.com/predict?age=" +
      age +
      "&sex=" +
      sex +
      "&cigs=" +
    ...
    .then((data) => {
      ...
      document.getElementById("percent").style.color =
        data.prediction[0] == 0 ? "#00FF00" : "#FF0000";
